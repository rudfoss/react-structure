...
export type HttpClient = <TResult>(pathSegments: string | string[], query?: Record<string, string>) => Promise<TResult>
...
export const jsonHttpClient =
	...
	async <TResult>(pathSegments: string | string[], query: Record<string, unknown> = {}): Promise<TResult> => {
		...
		const fullUrl = `${concatUrls([
			baseUrl,
			...(Array.isArray(pathSegments) ? pathSegments : [pathSegments])
